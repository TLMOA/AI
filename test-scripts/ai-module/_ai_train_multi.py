#!/usr/bin/env python3
"""通过前端同时训练多个模型并部署+预测。用法: python3 _ai_train_multi.py 'xgboost,LightGBM,1D-CNN'"""
...
def select_models(model_list):
    """勾选指定多个模型，取消其他"""
    all_models = ["xgboost", "LightGBM", "CatBoost", "hgb (梯度提升树)",
...
def wait_train(task_id, timeout=3600):
    import requests
...
def deploy_current():
    """部署 leaderboard 中所有 Ready 状态的模型"""
    deployed = 0
    end = time.time() + 120
    while time.time() < end:
        r = eval_clean("""async ()=>{
            ...
            for(const row of rows){
                if(row.textContent.includes('Ready') && row.textContent.includes('一键部署至 IoT')){
...
def get_predict_result(timeout=180):
    ...
    while time.time() < end:
        ...
        if 'BOX:' in r:
            ...
            return fault, total, nmodels
        time.sleep(3)
    return None, None, None

def main():
    models = sys.argv[1].split(",")
    print(f"=== 组合训练: {models} ===", flush=True)
    goto_home()
    if not select_train_data():
        print("FAIL: 选择数据失败", flush=True); sys.exit(1)
    ...
    data = wait_train(tid, 3600)
    if not data or data.get("status") != "completed":
        print("FAIL: 训练未完成", flush=True)
        print(json.dumps(data, ensure_ascii=False)[:600] if data else "no data", flush=True)
        sys.exit(1)
    ...
    if not select_predict_folder_file():
        ...
        if submit_predict():
            ...
            fault, total, nmodels = get_predict_result(180)
            print(f"预测结果: {nmodels} 个模型, {fault}/{total} 条故障", flush=True)
        else:
            print("FAIL: 预测提交失败", flush=True)
    res = {"models": models, "train": data, "predict_fault": fault if 'fault' in locals() else None,
           "predict_total": total if 'total' in locals() else None, "nmodels": nmodels if 'nmodels' in locals() else None}
    with open("/tmp/ai_multi_result.json", "w") as f:
