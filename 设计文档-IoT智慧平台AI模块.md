...
IoT 智慧平台 AI 模块是一个面向工业物联网场景的数据管理与智能分析平台。核心目标是构建"数据接入与转换 → 标签与特征 → 模型训练/推理 → 预警反馈"的完整闭环能力，最终以独立微服务形态嵌入 JetLinks 生态。
...
| 版本 | 代号 | 目标 | 状态 |
|------|------|------|------|
| V1 | 基础平台 | 数据导出/转换/标签/NiFi 对接/前端闭环 | 已封板 |
| V2 | Hadoop + AI | 大数据生态集成、联邦学习框架、部署模式 | 进行中 |
...
- **NiFi 模式**：NiFiExecutor，通过 NiFi REST API 驱动真实 NiFi 流程组执行，适合生产环境
...
- 预置容器/预置 Flow 方式运行
- Docker Compose 编排，独立 `iot-nifi.service`
- 时区同步（`TZ=Asia/Shanghai`）
- 宿主路径 ↔ 容器路径自动映射
...
- `NiFiClient` 类：支持登录、Token 管理、SSL 配置
- 流程组状态查询、启停
- 主动线程数监控
